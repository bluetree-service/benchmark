 ...
 * @link https://github.com/chajr/class-benchmark/wiki/ClassBenchmark%5CDebug%5CTracer
 */
...
class Tracer
{
    ...
    public static function marker($data)
    {
        ...
        if (isset($data[1][0]['args']) && is_array($data[1][0]['args'])) {
            foreach ($data[1][0]['args'] as $arg => $val) {
                self::_setObjectArgs($data, $val, $arg);
            }
        }
    ...
    /**
     * check if argument is object, serialize it and add to list
     * 
     * @param array $data
     * @param mixed $val
     * @param string $arg
     * @return array
     */
    protected static function _setObjectArgs(array $data, $val, $arg)
    {
        if (is_object($val)) {
            try {
